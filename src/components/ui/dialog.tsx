...
export function Modal({
  ...
  if (isMobile && !desktopOnly) {
    return (
      ...
      >
        <Drawer.Portal>
          <Drawer.Overlay className="fixed inset-0 z-50 bg-transparent w-full max-w-full  bg-opacity-10 backdrop-blur" />
          <Drawer.Content
            ...
            className={cn(
              "fixed w-screen max-w-full bottom-0 left-0 right-0 z-50 flex flex-col",
              "rounded-t-[10px] border-t border-neutral-200 bg-background",
              className,
            )}
          >
    ...
    >
      <Dialog.Portal>
        <Dialog.Overlay
          ...
          id="modal-backdrop"
          className="animate-fade-in fixed inset-0 z-40 bg-transparent bg-opacity-50 backdrop-blur-md"
        />
        <Dialog.Content
          ...
          className={cn(
            ...
            "border border-neutral-200 bg-background p-0 shadow-xl sm:rounded-2xl",
